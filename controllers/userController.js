...
class UserController {
            ...
            if (name && email && password && password_confirmation && tc) {
                if (password === password_confirmation) {
                    try {
                        ...
                        console.log(error)
                        res.status(422).send({ "status": "Failed", "message": "Unable to Registration" })
                    }
                } else {
                    res.status(422).send({ "status": "Failed", "message": "Password and Confirm Password doesn't match" })
                }
            } else {
            ...
            if (email && password) {
                ...
                if (user != null) {
                    ...
                    if ((email === user.email) && isMatch) {
                        ...
                        res.status(422).send({ "status": "Failed", "message": "Email or password is not a valid" })
                    }
                } else {
                    res.status(422).send({ "status": "Failed", "message": "You are not a registered user" })
                }
            } else {
                res.status(422).send({ "status": "Failed", "message": "All fields are required" })
            }
    ...
    static changeUserPassword = async (req, res) => {
        ...
        if (password && password_confirmation) {
            if (password !== password_confirmation) {
                res.status(422).send({ "status": "Failed", "message": "New Password and Confirm Password doesn't match" })
            } else {
                const salt = await bcrypt.genSalt(10)
                ...
                res.status(200).send({ "status": "Success", "message": "Password change success", })
            }
        } else {
            res.status(422).send({ "status": "Failed", "message": "All fields are required" })
        }
    ...
    static sendUserPasswordResetEmail = async (req, res) => {
        ...
        if (email) {
            ...
            if (user) {
                ...
                res.status(401).send({ "status": "failed", "message": "Email doesn't exists", })
            }
        } else {
